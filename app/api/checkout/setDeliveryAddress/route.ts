...
export async function POST(req: Request) {
  try {
    const {cart_id,city,country_code,firstname,lastname,telephone,street,postcode} = await req.json();
    const mutation = setDeliveryAddressGQL({
      cart_id,city,country_code,firstname,lastname,telephone,street,postcode
    });
